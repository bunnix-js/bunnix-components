...
test("Playground sidebar groups multi-component pages into nested items", () => {
  ...
  assert.match(appSource, /key: "inputs",\s*text: "Inputs",[\s\S]*children: \[[\s\S]*key: "text-input", text: "TextInput"[\s\S]*key: "text-area", text: "TextArea"[\s\S]*key: "currency-input", text: "CurrencyInput"[\s\S]*key: "select", text: "Select"[\s\S]*key: "checkbox", text: "CheckBox"[\s\S]*key: "switch", text: "Switch"[\s\S]*key: "segmented-picker", text: "SegmentedPicker"[\s\S]*key: "slider", text: "Slider"[\s\S]*\]/m);
...
test("Playground app routes every nested child key to its dedicated page builder", () => {
  ...
  assert.match(appSource, /if \(item === "currency-input"\) return CurrencyInputPage\(\);/);
...
test("Split playground page files export the expected child page builders", () => {
  ...
  assert.match(inputsPageSource, /export function CurrencyInputPage\(\) \{/);
