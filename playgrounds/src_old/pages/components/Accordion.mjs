...
export default function AccordionPage() {
  ...
  const accordionSnippet = [
    ...
    "    title: \"What's an accordion?\",",
    "    icon: \"icon-info\",",
    "    description: \"An accordion groups content into collapsible sections, with a title, a toggle icon, and controlled spacing.\"",
    "  },",
    "  {",
    "    title: \"When should it be used?\",",
    "    icon: \"icon-question-circle\",",
    "    description: \"Use it when users only need a few key details at a time and you want to reduce scroll length.\"",
    "  },",
    "  {",
    "    title: \"Does it allow only one open item?\",",
    "    icon: \"icon-merge\",",
    "    description: \"This variant keeps a single item open at a time; clicking another header collapses the current one.\"",
    "  },",
    "  {",
    "    title: \"How to signal expansion?\",",
    "    icon: \"icon-sections\",",
    "    description: \"Use a clear icon with a rotation or state change so users can immediately read the toggle action.\"",
    "  }",
    "];",
    "",
    "AccordionGroup({ items: accordionItems });"
  ].join("\n");
  const accordionHtml = Prism.highlight(accordionSnippet, Prism.languages.javascript, "javascript");
    ...
    div({ class: "column-container gap-sm" }, [
      PageSection({ title: "Multi-Item Group", stickyOffset: headerOffset }, [
        div({ class: "column-container gap-md" }, [
          AccordionGroup({ items: accordionItems }),
          CodeBlock({ html: accordionHtml, language: "js", wrap: true })
        ])
      ])
    ])
  ]);
