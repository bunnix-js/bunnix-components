...
export default function DropdownMenu({ items = [], id, align = "left", placeholder = "Select option..." }) {
      ...
      div({ class: "row-container items-center gap-sm no-margin" }, [
        ...
        span({ 
          class: useMemo([currentIcon, isDestructive], (icon, destructive) => 
            `icon ${icon} ${destructive ? 'bg-destructive' : 'bg-primary'} ${!icon ? 'hidden' : ''}`
          ) 
        }),
    ...
    div({
      ...
      div({ class: "card column-container shadow gap-sm w-min-150 p-sm bg-base" },
        items.map((item) => {
          ...
          return button({
            class: isCurrent.map(active => `btn btn-flat justify-start w-full ${active ? 'selected' : ''}`),
            click: () => handleItemClick(item)
          }, [
            span({ class: `icon ${item.icon} ${item.destructive ? 'bg-destructive' : 'bg-primary'}` }),
            item.title
          ]);
        })
      )
